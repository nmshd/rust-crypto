...
// These types are ignored because they are neither used by any `pub` functions nor (for structs and enums) marked `#[frb(unignore)]`: `CGivenExpected`, `CId`
// These function are ignored because they are on traits that is not defined in current crate (put an empty `#[frb]` on it to unignore): `clone`, `clone`, `fmt`, `fmt`, `fmt`, `fmt`, `fmt`, `fmt`, `fmt`, `fmt`, `fmt`, `fmt`, `fmt`, `fmt`, `from`, `from`, `from`, `source`

// Rust type: RustOpaqueMoi<flutter_rust_bridge::for_generated::RustAutoOpaqueInner<CPath>>
abstract class CPath implements RustOpaqueInterface {
  PathBuf get path;
...
sealed class CalErrorKind with _$CalErrorKind {
  ...
  const factory CalErrorKind.ephemeralKeyError() =
      CalErrorKind_EphemeralKeyError;
