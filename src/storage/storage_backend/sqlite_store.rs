...
impl SqliteBackend {
    pub(super) fn new(path: impl AsRef<Path>) -> Result<Self, StorageBackendInitializationError> {
        ...
        tracing::warn!("opening sql db: {:?}", path);
        ...
        if !conn.table_exists(None, "keys").map_err(|e| {
            StorageBackendInitializationError::Sqlite(SqliteBackendError::InitialisationError(
                format!("Can't execute query: {:?}", e),
            ))
        })? {
            match conn.execute("CREATE TABLE keys (id TEXT PRIMARY KEY, provider TEXT, encryption_key_id TEXT, signature_key_id TEXT, data_blob BLOB);", ()) {
            Ok(_) => Ok(()),
            Err(e) => Err(StorageBackendInitializationError::Sqlite(SqliteBackendError::InitialisationError(format!("Can't execute query: {:?}", e)))),
        }?;
        }
