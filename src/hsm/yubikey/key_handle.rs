...
use x509_cert::der;
use yubikey::piv;
use yubikey::{piv::AlgorithmId, Error, MgmKey};
...
impl KeyHandle for YubiKeyProvider {
    ...
    fn sign_data(&self, data: &[u8]) -> Result<Vec<u8>, SecurityModuleError> {
        ...
        if !auth.is_ok() {
            return Err(SecurityModuleError::HsmError::Authentication);
        }
        ...
        match self.key_algo {
            Ecc => {
                // Sign data
                let signature =
                    piv::sign_data(&mut self.yubikey, data, AlgorithmId::EccP256, self.slot_id);
            ...
            _ => {
                return Err(SecurityModuleError::HsmError::UnsupportedFeature);
            }
        }
    }
        ...
        match self.key_algo {
            "Rsa" => {
                decrypted = piv::decrypt_data(
                    ...
                    piv::SlotId::Retired(self.slot_id),
                );
            }
            "Ecc" => {
                // TODO, not tested, might work
            }
            "_" => Err(SecurityModuleError::HsmError::UnsupportedFeature),
        }
